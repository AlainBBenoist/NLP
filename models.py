...
def score_models(models, loader):
    for model in models:
        ...
        for X_train, X_test, y_train, y_test in loader:
            logger.info('{:s} : fitting run #{:d}'.format(name, nrun))
            start = time.time()
...
def print_classification(reader, estimator, reduction=False) :
...
def predict_model(model, reader, categories=None) :
    """
    Fitting a model on the entire dataset
    """
    logger.info('Predict : '+model_name(model))
    print(categories)
    X_pred = [ list(reader.docs(fileids=[fileid]))
                for fileid in reader.fileids(categories=categories) ]
    y_pred = model.predict(X_pred)
    return (y_pred, X_pred) 

def predict_model2(model, X_pred) :
    logger.info('Predict : '+model_name(model))
    y_pred = model.predict(X_pred)
    return y_pred
...
def save_models(models, prefix) :
    """
    Saving a list of models in pickleformat for operational reuse
    """
    for model in models :
        name = model_name(model)
